...
if (isset($_GET['id'])) {
	...
	$sql = "UPDATE itr_gst SET status='reject' WHERE id ='$id'";
	$result = mysqli_query($conn, $sql);
	if ($result == 1) {
...
<?php
$conn = mysqli_connect('localhost', 'root', '', 'payindia');
$sql = "SELECT * FROM itr_gst where status='false'";
$result = mysqli_query($conn, $sql);
$ans = mysqli_fetch_all($result, MYSQLI_ASSOC);
?>
<!DOCTYPE html>
					...
					?>
						<tbody>
							<tr>
								...
								<td>
									<a href="http://localhost/Pay%20India%20Website/Admin/gst.php?id=<?php echo $ans[$x]['id']; ?>&r_id=<?php echo $ans[$x]['r_id'];?>"><input type="button" value="Reply"></a>
								</td>
							</tr>
					<?php
					}
