...
if (isset($_GET['id'])) {
    ...
    $sql = "UPDATE pan SET status='false' WHERE id ='$id'";
    $result = mysqli_query($conn, $sql);
    if ($result == 1) {
...
<?php
$conn = mysqli_connect('localhost', 'root', '', 'payindia');
$sql = "SELECT * FROM pan where status='reject'";
$result = mysqli_query($conn, $sql);
$ans = mysqli_fetch_all($result, MYSQLI_ASSOC);
?>
<!DOCTYPE html>
                    ...
                    ?>
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <a href="http://localhost/Pay%20India%20Website/Admin/newPanRejected.php?id=<?php echo $ans[$x]['id']; ?>"><input type="button" value="Retry"></a>
                                </td>
                            </tr>
                    <?php
                    }
