...
if (isset($_GET['id'])) {
	...
	$sql = "UPDATE votercard SET status='reject' WHERE id ='$id'";
	$result = mysqli_query($conn, $sql);
	if ($result == 1) {
...
<?php
$conn = mysqli_connect('localhost', 'root', '', 'payindia');
$sql = "SELECT * FROM votercard where status='false'";
$result = mysqli_query($conn, $sql);
$ans = mysqli_fetch_all($result, MYSQLI_ASSOC);
?>
<!DOCTYPE html>
...
<head>
	...
	<title>Voter Card Pending</title>
					...
					?>
						<tbody>
							<tr>
								...
								<td>
									<a href="http://localhost/Pay%20India%20Website/Admin/voterCardPending.php?id=<?php echo $ans[$x]['id']; ?>"><input type="button" value="Reject"></a>
								</td>
								<td>
									<a href="http://localhost/Pay%20India%20Website/Admin/voterCard.php?id=<?php echo $ans[$x]['id']; ?>&r_id=<?php echo $ans[$x]['r_id'];?>"><input type="button" value="Reply"></a>
								</td>
							</tr>
					<?php
					}
