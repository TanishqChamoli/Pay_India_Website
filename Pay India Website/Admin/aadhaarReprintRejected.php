...
if (isset($_GET['id'])) {
	...
	$sql = "UPDATE aadhar_reprint SET status='false' WHERE id ='$id'";
	$result = mysqli_query($conn, $sql);
	if ($result == 1) {
		$address = "http://localhost/Pay%20India%20Website/Admin/aadhaarReprintRejected.php";
...
<?php
$conn = mysqli_connect('localhost', 'root', '', 'payindia');
$sql = "SELECT * FROM aadhar_reprint where status='reject'";
$result = mysqli_query($conn, $sql);
$ans = mysqli_fetch_all($result, MYSQLI_ASSOC);
?>
<!DOCTYPE html>
					...
					?>
						<tbody>
							<tr>
								...
								<td>
									<a href="http://localhost/Pay%20India%20Website/Admin/aadhaarReprintRejected.php?id=<?php echo $ans[$x]['id']; ?>"><input type="button" value="Retry"></a>
								</td>
							</tr>
					<?php
					}
