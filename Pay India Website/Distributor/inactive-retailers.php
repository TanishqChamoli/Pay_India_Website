...
if (isset($_GET['id'])) {
	...
	$sql = "UPDATE retailers SET status='true' WHERE id ='$id'";
	$result = mysqli_query($conn, $sql);
	if ($result == 1) {
		$address = "http://localhost/Pay%20India%20Website/Distributor/inactive-retailers.php";
...
<?php
$conn = mysqli_connect('localhost', 'root', '', 'payindia');
$sql = "SELECT * FROM retailers where status='false'";
$result = mysqli_query($conn, $sql);
$ans = mysqli_fetch_all($result, MYSQLI_ASSOC);
?>
<!DOCTYPE html>
...
<head>
	...
	<title>Inactive Retailers</title>
					...
					<tbody>
						<?php foreach ($ans as $value) { ?>
							<tr>
								...
								<td><a href=<?php echo "http://localhost/Pay%20India%20Website/Distributor/inactive-retailers.php?id=" . $value['id'] . ""; ?>><input type="button" value="Reactivate"></a></td>
